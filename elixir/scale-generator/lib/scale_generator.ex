defmodule ScaleGenerator do
  ...
  Given the `tonic` "D" in the `scale` (), you
  ...
  def step(scale, tonic, step) do
    old_index = Enum.find_index(scale, &(&1 == tonic))
    new_index = rem(old_index + @step[step], length(scale))
    Enum.at(scale, new_index)
  end
  ...
  def chromatic_scale(tonic \\ "C") do
    ...
    @chromatic_scale
    |> Enum.split_while(&(&1 != tonic))
    |> then(fn {a, b} -> b ++ a ++ [tonic] end)
  end
